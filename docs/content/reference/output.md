...
describes what users get. Pick a format with `-o`, or let hdx choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
hdx <command> -o table     # a rounded, color-aware grid for reading
hdx <command> -o markdown  # a GitHub pipe table to paste into docs (alias: md)
hdx <command> -o list      # one record per section, easy on the eyes
hdx <command> -o jsonl     # one JSON object per line, for piping
hdx <command> -o json      # a single JSON array
hdx <command> -o csv       # spreadsheet friendly
hdx <command> -o tsv       # tab-separated
hdx <command> -o url       # just the URL column
hdx <command> -o raw       # the underlying bytes, unformatted
```
...
```bash
hdx <command> --fields id,title,url
```
...
```bash
hdx <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
hdx <command>            # a table, because this is a terminal
hdx <command> | wc -l    # JSONL, because this is a pipe
```
